...
class GameDetailAct : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val boardGameBggId = dataSource.getBGG(boardGameId)
    ...
    private fun loadImageFilesFromDatabase(gameId: Int) {
        val dataSource = BoardGameCollector(this)
        val imageFiles = dataSource.getGamesImage(gameId)
    ...
    private fun removeCurrentImage() {
        if (images.size > 1) {
            ...
            val imagePaths = dataSource.getGamesImage(gameId)
    ...
    private fun showImage() {
        val imageView = findViewById<ImageView>(R.id.detalGameImage)
        Log.d("image check",images.size.toString())
        if (images.isNotEmpty()) {
